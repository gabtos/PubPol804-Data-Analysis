...
(Taxing Jurisdiction, or local Fund)`) %>%
  ...
  #TODO WARNING not removing NA values might affect things down the line, uncomment filter below
  # ) %>%
  # 
...
state_year_revenue <- all_data_no_electric %>%
  group_by(State, `Fiscal Year`, Renewable) %>%
   mutate(`Amount ($2022)` = gsub("[,$]", "", `Amount ($2022)`)) %>%
  mutate(`Amount ($2022)` = as.numeric(`Amount ($2022)`)) %>%
  summarise(Total_Revenue = sum(`Amount ($2022)`), .groups = "drop") %>%
...
ggplot(colorado_rev_year, aes(x = factor(`Fiscal Year`), y = Revenue / 1e6, fill = factor(`Fiscal Year`))) +  # Map fill to Year
  geom_col() +
  geom_text(aes(label = scales::comma(Revenue / 1e6)),  # Format numbers with commas
...
for (state in selected_states) {
  ...
  max_value <- df_summary %>%
    filter(State == state) %>%
    pull(total_revenue) %>%
    max(na.rm = TRUE) / 1e6  # Convert to millions
  
  # Create the plot
  temp_plot <- df_summary %>%
    filter(State == state) %>%
    ggplot(aes(x = year, y = total_revenue / 1e6, color = energy_type)) +
    geom_line(size = 1) +
...
max_value_national <- max(df_national_summary$total_revenue, na.rm = TRUE) / 1e6
...
national_plot <- df_national_summary %>%
  ...
  labs(
    ...
    subtitle = "Source: Resources for the Future 2022",
    x = "Year",
    y = "Tax Revenue (Millions USD)",
    color = "Energy Source"
  ) +
...
# Save the plot
ggsave(filename = "figures/National_tax_trends_time_series_new.jpg", 
...
for (state in unique(filtered_state_year_revenue$State)) {
  ...
  state_table <- filtered_state_year_revenue %>%
    ...
    gt() %>%
    tab_header(
      title = paste(state, "Tax Revenue Difference Renewable vs Non-Renewable"),
      subtitle = "Difference = Renewable - Non-Renewable (USD)"
    ) %>%
    tab_caption("Source: Resources for the Future 2022") %>% 
  ...
  gtsave(state_table, filename = paste0("figures/", state,"_revenue_difference_table_new.png"))
}
...
energy_type_table <- energy_type_revenue %>%
  gt() %>%
  tab_header(
    title = "Total Tax Revenue by Energy Type",
    subtitle = "National tax revenue for all energy types"
  ) %>%
  tab_caption("Source: Resources for the Future 2022") %>% 
...
gtsave(energy_type_table, filename = "figures/national_energy_type_revenue_new.png")
...
gtsave(yearly_category_table, filename = "figures/national_yearly_revenue_renewable_v_nonrenewable_new.png")
```
...
gtsave(yearly_energy_table, filename = "figures/national_yearly_revenue_by_energy_type_new.png")
...
temp_plot <- ggplot(all_data, aes(x = Recipient, y = (all_data$`Amount ($2022)`/1e6))) +
  geom_col(fill= "#5690be") +
  theme_minimal() +
  labs(title = "National Tax Fund Distribution by Recipient Type",
       subtitle='Source: Resources for the Future 2022',
       y = "Amount (Million, 2022 Adjusted USD)") +
  theme_minimal() +
  ...
  # Save the plot for each state
  ggsave(filename = paste0("figures/national_recipient_bargraph_new.jpg"), plot = temp_plot, width = 8, height = 6, dpi = 300)
```
...
temp_plot <- all_data %>%  
  filter(State == "CO") %>%  
  ggplot(aes(x = Recipient, y = `Amount ($2022)` / 1e6)) +  # Use correct columns from the filtered data
  geom_col(fill = "#5690be") +  
  theme_minimal() +  
  labs(
    title = "Colorado Tax Fund Distribution by Recipient Type",
    subtitle = "Source: Resources for the Future 2022",
    y = "Revenue (Million, 2022 Adjusted USD)"
  ) +  
  theme(
...
# Save the plot
ggsave(filename = "figures/CO_recipient_bargraph_new.jpg", plot = temp_plot, width = 8, height = 6, dpi = 300)
```
...
temp_plot <- all_data %>% 
  filter(State == "TX") %>% 
  ggplot(aes(x = Recipient, y = (`Amount ($2022)`/1e6))) +
  geom_col(fill = "#5690be") +  
  theme_minimal() +
  ...
  coord_flip()
  ggsave(filename = paste0("figures/TX_recipient_bargraph_new.jpg"), plot = temp_plot, width = 8, height = 6, dpi = 300)
...
temp_plot <- all_data %>% 
  ...
  coord_flip()
  ggsave(filename = paste0("figures/NM_recipient_bargraph_new.jpg"), plot = temp_plot, width = 8, height = 6, dpi = 300)
...
#   ) %>% 
#   select(state, year, energy_revenue)
#   
...
#     # Remember, the other dataset is in thousands
...
```{r comparing all revenue vs energy revenue for all states}

# view(state_year_summary)

# state_data_energy_modified <- all_data %>%
#   rename(energy_revenue = `Amount ($2022)`) %>%
#   rename(year = `Fiscal Year`) %>%
#   rename(state = State) %>%
#   filter(year >= 2018 & year <= 2021) %>%
#   select(state, year, energy_revenue) %>%    
#   group_by(state, year) %>%
#   summarise(total_revenue = sum(energy_revenue), .groups = "drop") %>%
#   arrange(state, year)

state_data_energy_modified <- all_data %>%
  rename(energy_revenue = `Amount (nominal)`) %>%
  mutate(energy_revenue = gsub("[,$]", "", energy_revenue)) %>%
  mutate(energy_revenue = as.numeric(energy_revenue)) %>%
  rename(year = `Fiscal Year`) %>%
  rename(state = State) %>%
  filter(year >= 2018 & year <= 2021) %>%
  group_by(state, year) %>% 
  summarise(total_energy_revenue = sum(energy_revenue))

view(state_data_energy_modified)
